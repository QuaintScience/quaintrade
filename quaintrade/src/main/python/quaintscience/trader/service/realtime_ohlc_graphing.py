...
class OHLCRealtimeGrapher(TradeManagerService):
    ...
    def animate(self, ival):
        df = list(self.manager.get_tick_data_as_ohlc(from_redis=True).values())[0]
        self.ax.clear()
    ...
    def start(self):
        ...
        self.fig, self.axes = mpf.plot(list(self.manager.get_tick_data_as_ohlc(from_redis=True).values())[0],
                                       returnfig=True,
                                       type='candle',
                                       title='Live Data',
